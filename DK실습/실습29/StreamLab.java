...
public class StreamLab {
    public static void main(String args[]) {
        ...
        Arrays.stream(sa).sorted((e1, e2) -> e2.getScore() - e1.getScore()).forEach(System.out::println);

        System.out.println("=============================");

        // (3) 성적이 200 이상인 Trainee 객체들을 화면에 출력한다.
        Arrays.stream(sa).filter(e -> e.getScore() >= 200).forEach(System.out::println);

        System.out.println("=============================");


        // (4) 성적이 200 이상인 수강생들의 인원을 출력한다.
        System.out.println(Arrays.stream(sa).filter(e -> e.getScore() >= 200).count() + "명");

        System.out.println("=============================");

        // (5) 남학생의 Trainee 객체들을 화면에 출력한다.
        Arrays.stream(sa).filter(e -> e.isMale()).forEach(System.out::println);

        System.out.println("=============================");

        // (6) 남학생의 인원을 출력한다.
        System.out.println(Arrays.stream(sa).filter(e -> e.isMale()).count() + "명");

        System.out.println("=============================");

        // (7) 모든 학생들의 스코어만 출력한다.
        Arrays.stream(sa).map(e -> e.getScore() + "점").forEach(System.out::println);

        System.out.println("=============================");

        // (8) 모든 학생들의 스코어 합을 출력한다.
        System.out.println(Arrays.stream(sa).map(e -> e.getScore()).reduce((x , y) -> x + y).get() + "점");
    }
}
